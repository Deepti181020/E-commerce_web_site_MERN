...
const ShopContextProvider = (props) => {
    ...
    useEffect(() => {
        ...
        const fetchProducts = async () => {
          try {
            const response = await axios.get("https://localhost:4000/allproducts");
        ...
        const fetchCart = async () => {
          ...
          if (token) {
            try {
              const response = await axios.get("https://localhost:4000/getcart", {
                headers: {
...
const addToCart = async (itemId) => {
  ...
  if (token) {
    try {
      const response = await axios.post("https://localhost:4000/addtocart", 
...
const removeFromCart = async (itemId) => {
    ...
    if (token) {
      try {
        const response = await axios.post("https://localhodt:4000/removecart", 
          { itemId: itemId }, // Send the itemId in the request body
          {
            headers: {
              Accept: 'application/json',
